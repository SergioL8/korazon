...
class _UserProfileState extends State<UserProfile> {
  ...
  void _getUserEvents(List<String> eventUids) async {
    for (int i = 0; i < eventUids.length; i++) {
      ...
      try {
        final eventDocument = await FirebaseFirestore.instance.collection('events').doc(eventUid).get();
        ...
        showErrorMessage(context, content: 'There was an error loading the events. Please try again.');
      }
    }
  }
  ...
  void _getUserImages(List<String> imagePaths) async {
    for (int i = 0; i < imagePaths.length; i++) {
      ...
      try {
        ...
        showErrorMessage(context, content: 'There was an error loading the images. Please try again.');
      }
    }
  }
  ...
  void _getUserData(userUID) async {
    ...
    final userDocument = await FirebaseFirestore.instance.collection('users').doc(userUID).get();
    final userModel = UserModel.fromDocumentSnapshot(userDocument);

    if (userModel == null) {
      showErrorMessage(context, content: 'There was an eror loading the profile. Please try again.');
      return;
    }
      ...
      body: CustomScrollView(
        slivers: [
          ...
          SliverAppBar(
            ...
            expandedHeight: MediaQuery.of(context).size.width * (4 / 3) - MediaQuery.of(context).padding.top,
            floating: false,
            snap: false,
            pinned: false,
            flexibleSpace: FlexibleSpaceBar(
              background: Stack(
                children: [
                  _loadingImages ? LoadingImagePlaceHolder() :
                  IndexedStack(
                    index: _imageIndex,
                    children: [
                      if (images.isEmpty)
                        Container(color: korazonColor,)
                      else
                        for (MemoryImage? image in images)
                          if (image == null)
                            LoadingImagePlaceHolder()
                          else
                            AspectRatio( // crop the image to a 3:4 aspect ratio
                              aspectRatio: 3 / 4,
                              child: Stack(
                                fit: StackFit.expand,
                                children: [
                                  ...
                                  Container(
                                    decoration: BoxDecoration(
                                      gradient: LinearGradient(
                                        ...
                                        stops: [0.01, 0.05, 0.1, 0.15, 0.2, 0.3, 0.75],
                                      ),
                                    ),
                                  ),
                                ],
                      ...
                      children: List.generate(images.length, (index) {
                        return Expanded(
                          child: Padding(
                            ...
                            child: AnimatedContainer( // use to smoothly change the indicator
                              duration: const Duration(milliseconds: 300),
                              height: index == _imageIndex ? 4 : 3,
                              decoration: BoxDecoration(
                                color: index == _imageIndex ? Colors.white : Colors.white.withOpacity(0.5),
                  ...
                  SizedBox( // needed because the height of the row is 0, without it, the gesture detector becomes ultra thin and you are not clicking it
                    height: double.infinity,
                    child: Row( // needed to place the gesture detectors on the left and right of the screen
                      children: [
                        Expanded( // needed to space the gesture detectors evenly occupying the entire screen
                          child: GestureDetector( // needed to detect the tap on the left side of the screen
                        ...
                        Expanded(
                          child: GestureDetector( // needed to detect the tap on the right side of the screen
                            child: Container(
                              color: Colors.transparent,
                            ),
                            onTap: () {
                              if (_imageIndex < images.length-1 && images[_imageIndex+1] != null) {
                                setState(() {
                                  _imageIndex++;
                                });
                              } else {
                    ...
                    child: Row(
                      children: [
                        ...
                        : ElevatedButton(
                          onPressed: () {},
                          style: ButtonStyle(
                            backgroundColor: WidgetStatePropertyAll(korazonColorBM),
                            minimumSize: WidgetStatePropertyAll(const Size(30, 27)),
                          ),
                          child: Text(
                            'Follow',
                  ...
                  Padding(
                    ...
                    : Column(
                      children: [
                        ...
                        userData!.snapAcc == '' ? Container() :
                        Row(
                          children: [
                            Icon(
                              FaIcon(FontAwesomeIcons.snapchat).icon, 
                              color: korazonColorBM,),
                            const SizedBox(width: 7,),
                            Expanded(
                              child: SelectableText(
